...
class ImageClassifier:
    ...
    def train(self, training_set: List[str], iterations: int = 100, epsilon: float = 1e-6, kernel_type: str = "SVM_LINEAR"):
        ...
        if kernel_type == "SVM_LINEAR":
            ...
            slef._classifier.setKernel(cv2.ml.SVM_POLY)
        elif kernel_type == "SVM_RBF":
            self._classifier.setKernel(cv2.ml.SVM_RBF)
        elif kernel_type == "SVM_SIGMOID":
            self._classifier.setKernel(cv2.ml.SVM_SIGMOID)
        elif kernel_type == "SVM_CHI2":
            self._classifier.setKernel(cv2.ml.SVM_CHI2)
        elif kernel_type == "SVM_INTER":
            self._classifier.setKernel(cv2.ml.SVM_INTER)
